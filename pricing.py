...
def pricing():
    ...
    working_oil=[]
    for k,v in config['price_oil'].iteritems():
            working_oil.append((adjust_date_to_EOM(k), v))

    working_gas=[]
    for k,v in config['price_gas'].iteritems():
            working_gas.append((adjust_date_to_EOM(k), v))

    working_ngl=[]
    for k,v in config['price_ngl'].iteritems():
    ...
    prices.to_excel(r'C:\Users\WaltN\Desktop\GitHub\curvey\output\prices.xlsx')
    return prices